...
object DaoImpl extends Dao {
	...
	private val queryGetEntityFieldsById = Compiled((id: Column[Long]) =>
 		(for (i <- entities if i.id === id) yield i).map(e => (e.eType, e.name, e.location, e.x, e.y, e.radius, e.view_radius, e.voice_radius, e.max_speed)))
	...
	def addGameEntity(eType: EntityType,
		...
		val id = (entities.map(e => (e.eType, e.name, e.location, e.x, e.y, e.radius, e.view_radius, e.voice_radius, e.max_speed)) returning entities.map(_.id)) +=
				(eType.name, name, location, x, y, radius, viewRadius, voiceRadius, maxSpeed)
		GameEntity(id, false, eType, name, location, x, y, radius, viewRadius, voiceRadius, maxSpeed)
	}
 	...
 	def updateGameEntity(id: Long,
											 ...
											 voiceRadius: Double,
											 maxSpeed: Double) = db withDynTransaction {
		queryGetEntityFieldsById(id).update((eType.name, name, location, x, y, radius, viewRadius, voiceRadius, maxSpeed)) == 1
	}


	private def convertUser(d: (Long, Long, String, String, Long)) = User.tupled(d)

	private def convertEntity(d: (Long, String, String, String, Double, Double, Double, Double, Double, Double)) =
		GameEntity(d._1, false, EntityType.get(d._2), d._3, d._4, d._5, d._6, d._7, d._8, d._9, d._10)
}
